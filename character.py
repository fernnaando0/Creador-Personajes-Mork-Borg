...
class Character:
    ...
    def __init__(self):
        ...
        # Lore generado por IA (Modo Narrativo)
        self.lore = ""
    ...
    def get_ability_modifier(self, value_3d6):
        if 1 <= value_3d6 <= 4:
            ...
            return 0
        elif 13 <= value_3d6 <= 14:
            return 1
        elif 15 <= value_3d6 <= 16:
            return 2
        elif 17 <= value_3d6 <= 20:
            return 3
        return 0
    ...
    def set_lore(self, lore_text):
        """Almacena el transfondo generado por IA."""
        self.lore = lore_text

    def get_lore_prompt(self):
        """Devuelve el prompt para la IA con los datos del personaje."""
        return (
            f"Eres un escritor de la ambientación MÖRK BORG: un mundo oscuro, grotesco y apocalíptico, "
            f"donde la muerte es inminente y todo está condenado. El tono es brutal, poético y desesperado. "
            f"Genera un transfondo de MÁXIMO 5 líneas para este personaje. "
            f"El transfondo debe estar escrito en segunda persona ('Eres...', 'Fuiste...', 'Tu...'). "
            f"Debe integrar de forma narrativa y coherente estos tres elementos del personaje:\n"
            f"- Cuerpo roto: {self.broken_body}\n"
            f"- Mal hábito: {self.bad_habits}\n"
            f"- Historia perturbadora: {self.troubling_tale}\n"
            f"No hagas una lista. Escribe un párrafo o dos párrafos cortos. "
            f"Sin saludos, sin explicaciones, solo el transfondo."
        )

    def get_lore_str(self):
        """Muestra el personaje completo con el lore narrativo de la IA."""
        base = str(self)
        lore_block = (
            f"\n========================================\n"
            f" TRANSFONDO (GENERADO POR IA)\n"
            f"----------------------------------------\n"
            f"{self.lore if self.lore else '[ Sin transfondo generado ]'}\n"
            f"========================================"
        )
        return base + lore_block
